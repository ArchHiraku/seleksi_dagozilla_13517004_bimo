...
// choosing bomb_qty number of random bomb location, by permutation array of integers indicating the location possibilities and taking the first bomb_qty numbers
...
// What does this function do?
// call raiseMapValue on cells around [row][col]
...
// What does this function do?
// change [row][col] cell to be explored, halts if it is the bomb, and continue exploreMapFromPosition of its adjacent cells if the map.value is 0
...
int main()
{
  ...
  // ask for input until valid N and B is given, where it will call initiateMap(map,N,B)
  do
  {
  ...
  // What does do-while below do?
  // print the map, input a position from player, and process the game while the game has not ended yet
  do
  {
    printMap(map, N);
